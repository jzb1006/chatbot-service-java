...
- [ ] Hermes 未结束前，第一句完整句子已经触发腾讯云 `ACTION_SYNTHESIS`。
- [ ] 第一段腾讯云 binary PCM 返回后，后端立即编码并下发小智 binary Opus。
- [ ] 一轮回答只出现一次 `tts.start` 和一次 `tts.stop`。
- [ ] `abort` 后没有旧轮次 binary 继续下发。
- [ ] `mvn -pl chatbot-speech-api,chatbot-voice-gateway -am test` 通过。
- [ ] smoke 输出 `first_binary_ms`，可用于对比优化前后首包延迟。
